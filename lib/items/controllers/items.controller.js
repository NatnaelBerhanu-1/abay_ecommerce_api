...
exports.updateItem = async (id, item) => {
    try {
        ...
        if (itm == null) {
            ...
            await Item.update({ name: item.name, description: item.description, cityId: item.cityId, condition: item.condition, price: item.price, categoryId: item.categoryId, userId: item.userId, isActive: item.isActive }, { where: { id: id } });
            return 204;
        }
    } catch (error) {
        console.log(error);
